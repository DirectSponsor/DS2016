...
class InvitationsController extends DirectSponsorBaseController {
    ...
    public function store(Request $request, $pid){
        ...
        if ($project->isFullySubscribedWithRecipients()) {
            Session::put('error','The maximum number of recipients have already been assigned to this project!');
        ...
        if ($result instanceof Validator) {
            ...
            return redirect()->route('projects.invitations.create',$pid)
                        ->withErrors($result)
                        ->withInput();
        }
        ...
        if (config('app.debug')) {
            $link = "<a href='".URL::route('recipients.join',$invitation->id)."'>Recipient Register Link</a>";
