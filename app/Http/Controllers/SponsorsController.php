...
class SponsorsController extends DirectSponsorBaseController {
    ...
    public function index(){
        if ((Auth::user()->account_type == 'Admin') or (Auth::user()->account_type == 'Coordinator')) {
            // Continue
        } else {
            abort(403, 'Not authorised to view Sponsors List');
        }
    ...
    public function show($id){
        if ((Auth::user()->account_type == 'Admin') or (Auth::user()->account_type == 'Coordinator')) {
            ...
            abort(403, 'Not authorised to view Sponsor Detail');
        }
    ...
    public function suspend($id, $pid){
        if ((Auth::user()->account_type == 'Admin') or (Auth::user()->account_type == 'Coordinator')) {
            ...
            abort(403, 'Not authorised to Suspend Sponsor');
        }
        $sponsor = Sponsor::find($id);
        if(is_null($sponsor)){
            abort(404, 'Sponsor Not Found');
    ...
    public function forProject($pid){
        if ((Auth::user()->account_type == 'Admin') or (Auth::user()->account_type == 'Coordinator')) {
            // Continue
        } else {
            abort(403, 'Not authorised to View Project Sponsors');
        }
        if (Auth::user()->account_type == 'Coordinator') {
            $project = Project::find(Auth::user()->account->project_id);
        } else {
        ...
        $this->render('sponsors.index','Sponsors of the Project '.$project->name,
                array(
    ...
    public function projects($id){
        if ((Auth::user()->account_type == 'Admin') or (Auth::user()->account_type == 'Coordinator')) {
            ...
            abort(403, 'Not authorised to View Sponsor Projects');
        }
        $sponsor = Sponsor::find($id);
        if(is_null($sponsor)){
            abort(404, 'Sponsor Not Found');
