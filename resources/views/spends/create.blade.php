{!!  Form::open(array('route'=>array('projects.spends.store',$project->id)))  !!}
    ...
    {!!  Form::label('amount','Amount :'), Form::text('amount',Input::old('amount'),array('style'=>'width:70px')) !!} <p class="text-danger" style ="background:transparent"> {!!  $project->currency  !!} </p>
    ...
    {!!  Form::label('description','Description :'), Form::textarea('description',Input::old('description'))  !!}
        @if($errors->has('description')) {!!  $errors->first('description','<p class="text-danger"> :message </p>')  !!} @endif
        <div class="clear"></div>
