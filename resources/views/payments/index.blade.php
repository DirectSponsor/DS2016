...
if ($visitorAccountType == 'Coordinator') {
    if ($profileAccountType == 'Recipient') {
        $showControls = true;
    }
}
?>

@if(!count($payments)) <table class="table"> @else <table class="table data table-striped table-bordered"> @endif
    <thead>
        <tr>
            <?php switch($visitorAccountType){
                case 'Admin': ?>
                    <th>Due Date</th>
                    <th>For Month</th>
                    <th>Project</th>
        ...
        <tr>
            ...
            <td>
                ...
                @if($payment->overduePayment())
                    {!!  Form::open(array('class'=>'inline', 'method'=>'post','route'=>array('payments.late',$payment->id)))  !!}
                    {!! Form::label('explanation','Explanation :'),
                        Form::textarea('explanation', Input::old('explanation'), ['size' => '30x5'])
                        !!}
                        @if($errors->has('explanation')) {!!  $errors->first('explanation','<span class="text-danger"> :message </span>')  !!} @endif
                    {!!  Form::submit('Confirm Late'),
                    Form::close()  !!}
                @endif
            </td>
            @endif
    ...
    @foreach($payments as $payment)
        <tr>
            ...
            <td>
                ...
                @endif
                @if($payment->overduePayment())
                    {!!  Form::open(array('class'=>'inline', 'method'=>'delete','route'=>array('payments.late',$payment->id))),
                        Form::submit('Confirm Late'),
                    Form::close()  !!}
                @endif
            </td>
